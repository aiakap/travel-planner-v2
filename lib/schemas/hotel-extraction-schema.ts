...
export const hotelExtractionSchema = z.object({
  confirmationNumber: z.string().describe("Booking confirmation number (e.g., ABC123456789)"),
  guestName: z.string().describe("Guest name from reservation (e.g., John Doe or DOE/JOHN)"),
  ...
  checkInTime: z.string().describe("Check-in time (e.g., 3:00 PM) or empty string if not specified"),
  checkOutDate: z.string().describe("Check-out date in ISO format (YYYY-MM-DD)"),
  checkOutTime: z.string().describe("Check-out time (e.g., 11:00 AM) or empty string if not specified"),
  roomType: z.string().describe("Room type (e.g., Deluxe King, Standard Queen) or empty string if not provided"),
  ...
  bookingDate: z.string().describe("Date of booking in ISO format or empty string if not provided"),
});
