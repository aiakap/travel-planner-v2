...
const CATEGORY_SEEDS: CategorySeed[] = [
  {
    ...
    children: [
      ...
      },
      {
        ...
        sortOrder: 3,
        children: [
          { name: "Budget", slug: "budget-level", level: 2, sortOrder: 1 },
          { name: "Mid-Range", slug: "mid-range-level", level: 2, sortOrder: 2 },
          { name: "Luxury", slug: "luxury-level-value", level: 2, sortOrder: 3 },
          { name: "Ultra-Luxury", slug: "ultra-luxury-level", level: 2, sortOrder: 4 }
        ]
      },
      {
...
async function seedCategories() {
  console.log('🌱 Starting profile category seed...\n');

  try {
    // Check if categories already exist
    const existingCount = await prisma.profileCategory.count();
    if (existingCount > 0) {
    ...
    const totalCount = await prisma.profileCategory.count();
    console.log(`\n✅ Category seed complete! Created ${totalCount} categories.`);
  } catch (error) {
